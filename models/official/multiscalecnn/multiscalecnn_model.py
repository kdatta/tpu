...
def preconv_kernel(prefix, inputs, pool_size, stride, data_format, preconv_counter):
...
def conv_kernel(name_prefix, inp, nIn, nOut, kH, kW, dH, dW, padType, padding, pV, data_format):
...
def inner_product(prefix, inp, nIn, nOut):
  name = prefix + '_fc'
  # with tf.name_scope(name) as scope:
...
def pc_par_conv_pool(prefix, inp, kHpc, kWpc, stride, nIn,
  ...
  # conv1 + relu1
  conv1 = conv_kernel(prefix + '_conv1', preConv_pool, nIn, nOut, kH, kW, dH, dW, padType, padding, pV, data_format)
  # conv1-1 + relu1-1
  conv2 = conv_kernel(prefix + '_conv2', conv1, nOut, nOut, kH, kW, dH, dW, padType, padding, pV, data_format)
  # conv1-2 + relu1-2
  conv3 = conv_kernel(prefix + '_conv3', conv2, nOut, nOut, kH, kW, dH, dW, padType, padding, pV, data_format)
...
def conv_pool(inp, nIn, nOut, kH, kW, dH, dW, kHp, kWp, dHp, dWp, padType, padding, pV, data_format):
  ...
  with tf.name_scope(name) as scope:
    # conv1 + relu1
    conv1 = conv_kernel(name + '_conv1', inp, nIn, nOut, kH, kW, dH, dW, padType, padding, pV, data_format)
    # conv1-1 + relu1-1
    conv2 = conv_kernel(name + '_conv2', conv1, nOut, nOut, kH, kW, dH, dW, padType, padding, pV, data_format)
    # conv1-2 + relu1-2
    conv3 = conv_kernel(name + '_conv3', conv2, nOut, nOut, kH, kW, dH, dW, padType, padding, pV, data_format)
    # Pooling
...
def model_generator(block_fn, layers, num_classes,
  ...
  def model(inputs, is_training):
    ...
    mergedSummaryConv = conv_kernel('mergedSummaryConv', col_merge, 208, 1024, 1, 1, 1, 1, 'SAME', False, 0, None, data_format=data_format)

    # poolMergedSummaryConv
    poolMergedSummaryConv = max_pool_kernel('mergedSummaryConv', mergedSummaryConv, 2, 2, 2, 2)
